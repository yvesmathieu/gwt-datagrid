...
public class ContactDatabase {
	 ...
	 */
	private final Map<Integer, Set<ContactInfo>> friendsMap = new HashMap<Integer, Set<ContactInfo>>();
	...
	public Set<ContactInfo> queryFriends(ContactInfo contact) {
		...
		if (friends == null) {
			...
			for (int i = 0; i < friendCount; i++) {
				friends.add(dataProvider.getList().get(
						Random.nextInt(numContacts)));
			}
